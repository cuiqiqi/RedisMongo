...
//		point.setLat(31.979234);
	...
	public List<Object> withinPolygon(){
		...
		for(DBObject obj : listfinal)
			System.out.println("第一个多边形"+obj);

		//第二个矩形
		List<double[]> list2 = new ArrayList<>();
		list2.add(new double[]{90,60});
		list2.add(new double[]{90,0});
		list2.add(new double[]{130,0});
		list2.add(new double[]{130,60});
		list2.add(new double[]{90,60});
		List<DBObject> listfinal2 = userService.withinPolygon("point.test",  "loc",
				list2,  field, query, limit);
		list.add(listfinal2);
		for(DBObject obj : listfinal2)
			System.out.println("第二个多边形"+obj);
